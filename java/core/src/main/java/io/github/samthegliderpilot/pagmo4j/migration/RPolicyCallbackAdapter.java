package io.github.samthegliderpilot.pagmo4j.migration;

import io.github.samthegliderpilot.pagmo4j.*;

/**
 * SWIG director adapter for user-defined replacement policies.
 *
 * <p>Extend this class and override {@link #replace} to implement a custom
 * replacement policy. Pass it to
 * {@code archipelago.pushBackIsland(..., IRPolicy, ISPolicy, ...)} or wrap it
 * directly: {@code new ManagedRPolicy(adapter)}.
 *
 * <p>Inheriting from {@link RPolicyCallback} wires SWIG director dispatch so
 * native pagmo calls back into this Java implementation during migration.
 */
public abstract class RPolicyCallbackAdapter extends RPolicyCallback implements IRPolicy {
    private volatile Throwable deferredEx;

    @Override
    public final IndividualsGroup replace(
        IndividualsGroup incoming,
        long n_f, long n_ec, long n_ic, long n_obj,
        long pop_size, DoubleVector tol,
        IndividualsGroup current) {
        try {
            IndividualsGroup result = replaceManaged(incoming, n_f, n_ec, n_ic, n_obj, pop_size, tol, current);
            if (result == null) throw new NullPointerException("replace() returned null");
            return result;
        } catch (Throwable ex) {
            if (deferredEx == null) deferredEx = ex;
            return new IndividualsGroup();
        }
    }

    protected abstract IndividualsGroup replaceManaged(
        IndividualsGroup incoming,
        long n_f, long n_ec, long n_ic, long n_obj,
        long pop_size, DoubleVector tol,
        IndividualsGroup current);

    @Override
    public String get_name() {
        try { return IRPolicy.super.get_name(); } catch (Throwable ex) { if (deferredEx == null) deferredEx = ex; return "Java r_policy"; }
    }

    @Override
    public String get_extra_info() {
        try { return IRPolicy.super.get_extra_info(); } catch (Throwable ex) { if (deferredEx == null) deferredEx = ex; return ""; }
    }

    @Override
    public boolean is_valid() {
        try { return IRPolicy.super.is_valid(); } catch (Throwable ex) { if (deferredEx == null) deferredEx = ex; return false; }
    }

    public Throwable consumeDeferredException() {
        Throwable ex = deferredEx;
        deferredEx = null;
        return ex;
    }
}
