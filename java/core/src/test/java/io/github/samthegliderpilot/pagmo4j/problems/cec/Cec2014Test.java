...
class Cec2014Test extends ProblemTestBase {
    @Override public IProblem createStandardProblem() { return new cec2014(1L, 5L); }
    @Override @Test protected void boilerPlate() {
        try (cec2014 p = new cec2014(1L, 5L)) { assertFalse(p.get_name().isEmpty()); assertEquals(1L, p.get_nobj()); }
    }
    @Override @Test protected void optimizing() {
        try (cec2014 p = new cec2014(1L, 5L); de a = new de(20L); population pop = new population(p, 32L, 2L)) {
            a.set_seed(2L);
